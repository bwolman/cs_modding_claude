// Decompiled from Game.dll — Game.Citizens.Citizen
// The core component for every citizen entity in the simulation.
...
public struct Citizen : IComponentData, IQueryTypeParameter, ISerializable
{
    ...
    public int GetEducationLevel()
    {
        if ((m_State & CitizenFlags.EducationBit3) != CitizenFlags.None)
            return 4;
        return (((m_State & CitizenFlags.EducationBit1) != CitizenFlags.None) ? 2 : 0)
             + (((m_State & CitizenFlags.EducationBit2) != CitizenFlags.None) ? 1 : 0);
    }

    public void SetAge(CitizenAge newAge)
    {
        m_State = (CitizenFlags)((int)((uint)(m_State & ~(CitizenFlags.AgeBit1 | CitizenFlags.AgeBit2))
            | (uint)(((newAge & CitizenAge.Adult) != CitizenAge.Child) ? 1 : 0))
            | (((int)newAge % 2 != 0) ? 2 : 0));
    }

    public CitizenAge GetAge()
    {
        return (CitizenAge)(2 * (((m_State & CitizenFlags.AgeBit1) != CitizenFlags.None) ? 1 : 0)
            + (((m_State & CitizenFlags.AgeBit2) != CitizenFlags.None) ? 1 : 0));
    }
}
