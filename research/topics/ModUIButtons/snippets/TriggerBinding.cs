// Decompiled from: Colossal.UI.Binding.dll -> Colossal.UI.Binding.TriggerBinding (all arities)
// JS -> C# event bindings. TypeScript calls trigger(group, name, ...args) to invoke.
//
// Arities:
//   TriggerBinding              — Action (no args)        — extends BindingBase directly
//   TriggerBinding<T>           — Action<T>               — extends RawTriggerBindingBase
//   TriggerBinding<T1,T2>       — Action<T1,T2>           — extends RawTriggerBindingBase
//   TriggerBinding<T1,T2,T3>    — Action<T1,T2,T3>        — extends RawTriggerBindingBase
//   TriggerBinding<T1,T2,T3,T4> — Action<T1,T2,T3,T4>    — extends RawTriggerBindingBase
//
// Usage:
//   AddBinding(new TriggerBinding("MyMod", "ToggleTool", OnToggle));
//   AddBinding(new TriggerBinding<string>("MyMod", "SetName", OnSetName));
//   AddBinding(new TriggerBinding<int, int>("MyMod", "SetPos", OnSetPos));
...
// === No-arg TriggerBinding ===
// Note: this one extends BindingBase directly (no RawTriggerBindingBase)
// because it doesn't need to read any arguments from JSON.
public class TriggerBinding : BindingBase
{
	...
	private void Callback()
	{
		...
		{
			BindingBase.log.Error(exception, "Error in trigger binding callback '" + base.path + "'");
		}
	}
...
public class TriggerBinding<T1, T2, T3> : RawTriggerBindingBase
{
	...
	public TriggerBinding(string group, string name, [NotNull] Action<T1, T2, T3> callback,
		IReader<T1> reader1 = null, IReader<T2> reader2 = null, IReader<T3> reader3 = null)
		: base(group, name)
	{
		m_Callback = callback ?? throw new ArgumentNullException("callback");
		m_Reader1 = reader1 ?? ValueReaders.Create<T1>();
...
// === 4-arg TriggerBinding<T1, T2, T3, T4> ===
public class TriggerBinding<T1, T2, T3, T4> : RawTriggerBindingBase
{
	...
	public TriggerBinding(string group, string name, [NotNull] Action<T1, T2, T3, T4> callback,
		IReader<T1> reader1 = null, IReader<T2> reader2 = null, IReader<T3> reader3 = null, IReader<T4> reader4 = null)
		: base(group, name)
	{
		m_Callback = callback ?? throw new ArgumentNullException("callback");
		m_Reader1 = reader1 ?? ValueReaders.Create<T1>();
